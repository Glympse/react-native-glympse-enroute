...
export default function App() {
  ...
  useEffect(() => {
    ...
    EnRouteWrapper.instance().getEnRoute().isLoginNeeded().then((loginNeeded) => {
      if (loginNeeded) {
        setIsLoggedIn(false);
      } else {
        setIsLoggedIn(true);
    ...
    listenerSubscription.current = GlympseEnroute.onEnRouteEvent((glympseEvent) => {
      ...
      if (glympseEvent.listener === LISTENER_ENROUTE_MANAGER) {
        if (0 != (glympseEvent.events & ENROUTE_MANAGER_SYNCED)) {
          // Show Tasks screen
          setIsLoggedIn(true);
          setIsLoading(false);
        } else if (0 != (glympseEvent.events & ENROUTE_MANAGER_AUTHENTICATION_NEEDED)) {
          // Show login screen
          setIsLoggedIn(false);
          setIsLoading(false);
        } else if (0 != (glympseEvent.events & ENROUTE_MANAGER_LOGGED_OUT)) {
          setIsLoggedIn(false);
          setIsLoading(false);
        }
      }
