...
const TaskDetailsScreen: React.FC<Props> = ({ route }) => {
  ...
  useEffect(() => {
    const loadTask = async () => {
      const taskData = await EnRouteWrapper.instance()
        .getEnRoute()
      ...
      {task ? (
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.card}>
            ...
            {task.operation.ticket.destination ? (
              <InfoRow
                label="Destination"
                value={task.operation.ticket.destination.name}
              />
            ) : null}
            {task.operation.ticket.destination ? (
              <InfoRow
                label="Location"
                value={`${task.operation.ticket.destination.latitude}, ${task.operation.ticket.destination.longitude}`}
              />
            ) : null}
          ...
          <View style={styles.card}>
            ...
            <InfoRow
              label="Arrived Time"
              value={formatTime(task.arrivedTime)}
            />
            <InfoRow
              label="Completed Time"
              value={formatTime(task.completedTime)}
              isLast
            />
          </View>
          ...
          {task.operation ? (
            <View style={styles.card}>
              ...
              <InfoRow
                label="Start Time"
                value={formatTime(task.operation.startTime)}
                isLast
              />
            </View>
          ) : null}
          ...
          {task.operation.ticket ? (
            <View style={styles.card}>
              ...
              <InfoRow
                label="Travel Mode"
                value={
                  TravelMode[task.operation.ticket.travelMode] || 'Unknown'
                }
              />
              <InfoRow
                label="ETA"
                value={formatEta(task.operation.ticket.eta)}
              />
              {task.operation.ticket.track ? (
                <InfoRow
                  label="Track distance"
                  value={`${task.operation.ticket.track.distance}m`}
                />
              ) : null}
              {task.operation.ticket.route ? (
                <InfoRow
                  label="Route distance"
                  value={`${task.operation.ticket.route.distance}m`}
                />
              ) : null}
              <InfoRow
                label="Visibility"
                value={task.operation.ticket.visibility}
                isLast
              />
              <TouchableOpacity
                ...
                onPress={() =>
                  EnRouteWrapper.instance()
                    .getEnRoute()
                    .taskManager.getTicketWithFullLocationArray(task.id)
                    .then((updatedTicket) => {
              ...
              >
                <Text style={styles.buttonText}>
                  Refresh Ticket Location Tracks
                </Text>
              </TouchableOpacity>
            </View>
          ) : null}
            ...
            {task.metadata.length > 0 ? (
              task.metadata.map((item, index) => (
                <InfoRow
                  ...
                  isLast={index === task.metadata.length - 1}
                />
              ))
            ) : (
